...
if (!user) return message.reply({embed:{color:'RED',description:'mention a user to unmute '}})
...
if(!reason) return message.reply({embed:{color:"RED",description:"Mention a reason to unmute the user"}})
 ...
 try{
      await member.send(embed)
           ...
           member.roles.remove(muterole.id)
            message.channel.send({embed:{color:"GREEN",description:`${user} has been unmuted by ${message.author}`}});
 }
        }
}